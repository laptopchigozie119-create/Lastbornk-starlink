...
function App() {
  ...
  return <div className="app-shell">
    ...
    {modal?.type === 'book' && <BookingModal host={modal.host} user={user} close={()=>setModal(null)} success={(booking)=>{notify(`Voucher ${booking.code} purchased for ${money(booking.amount)}.`,'success','voucher');if(booking.connection?.connected)notify(`Connected to ${booking.hostName} successfully.`,'success','connection');setModal({type:'confirmed',booking});load()}}/>}
    ...
    {modal?.type === 'hardware' && <HardwareConfigModal close={()=>setModal(null)} notify={notify}/>}
...
function BookingModal({host,user,close,success}) { const [hours,setHours]=useState(1); const [busy,setBusy]=useState(false); const [error,setError]=useState(''); const total=hours*host.price; const submit=async()=>{setBusy(true);setError('');try{if(host.production){const params=new URLSearchParams(location.search);const d=await secureApi('/api/vouchers/purchase',{method:'POST',body:JSON.stringify({hostId:host.id,clientMac:params.get('mac')||null,durationMinutes:hours*60,speedProfile:'5M/5M'})});let connection=d.routerActivation?.simulated?null:{connected:false,physical:true,status:d.routerActivation?.status};let connectedSession=d.session;if(d.routerActivation?.simulated){try{const connected=await secureApi(`/api/vouchers/${d.session.id}/connect`,{method:'POST'});connection=connected.connection;connectedSession=connected.session}catch(connectError){connection={connected:false,warning:connectError.message}}}success({...connectedSession,code:connectedSession.access_code,hostName:host.name,hours,amount:Number(connectedSession.amount_paid),routerLogin:d.routerLogin,routerActivation:d.routerActivation,connection})}else{const d=await api('/api/bookings',{method:'POST',body:JSON.stringify({hostId:host.id,hours})});success(d.booking)}}catch(e){setError(e.message)}finally{setBusy(false)}}; return <Modal close={close} size="booking-modal"><p className="eyebrow">CONFIRM CONNECTION</p><div className="book-host"><div className="host-avatar">{host.avatar}</div><div><h2>{host.name}</h2><p><MapPin size={14}/>{host.area} · {host.distance} km away</p></div></div><div className="connection-stats"><span><Gauge/><b>{host.speed} Mbps</b><small>Speed</small></span><span><Star/><b>{host.rating}</b><small>Rating</small></span><span><Users/><b>{host.spots}</b><small>Spots left</small></span></div><label className="field-label">How long do you need?</label><div className="duration-pills">{[1,2,3,5].map(n=><button className={hours===n?'active':''} onClick={()=>setHours(n)} key={n}>{n} hr{n>1?'s':''}</button>)}</div><div className="order-summary"><div><span>{money(host.price)} × {hours} hour{hours>1?'s':''}</span><b>{money(total)}</b></div><div><span>Service fee</span><b>₦0</b></div><div className="total"><span>Total</span><b>{money(total)}</b></div></div><div className="wallet-pay"><Wallet/><div><b>Lastbornk wallet</b><small>Balance: {money(user.balance)}</small></div><Check/></div>{error&&<p className="form-error">{error}</p>}<button className="primary wide" onClick={submit} disabled={busy}>{busy?<Loader2 className="spin"/>:`Pay ${money(total)} & connect`}</button></Modal> }
function ConfirmedModal({booking,close,onViewDashboard}) { const loginUrl=new URLSearchParams(location.search).get('loginUrl'); const continueFlow=()=>{if(loginUrl&&booking.routerLogin)submitRouterLogin(loginUrl,booking.routerLogin.username,booking.routerLogin.password);else onViewDashboard()}; return <Modal close={close}><div className="success-ring"><Check/></div><h2>Voucher activated!</h2><p>{booking.connection?.connected?'Mock MikroTik activation completed and your device is now Connected. Live usage telemetry has started.':booking.connection?.warning?`Voucher purchased successfully. Connection can be retried from Activity: ${booking.connection.warning}`:booking.routerActivation?.simulated?'Mock MikroTik activation completed successfully. Your voucher is active and ready for testing.':loginUrl?'Your voucher is ready. Continue to authenticate this device.':'Show this access code to the host when you arrive.'}</p><div className="access-code"><span>6-DIGIT VOUCHER PIN</span><strong>{booking.code}</strong></div><div className="confirm-info"><Wifi/><div><b>{booking.hostName}</b><small>{booking.hours} hour session · {money(booking.amount)}</small></div></div><button className="primary wide" onClick={continueFlow}>{loginUrl?'Connect to WiFi':'View dashboard'}</button></Modal> }
...
function AuthScreen() {
  ...
  const submit=async e=>{e.preventDefault();setBusy(true);setError('');try{const normalizedPhone=normalizeNigerianPhone(phone);if(!sent){setPhone(normalizedPhone);const{error}=await supabase.auth.signInWithOtp({phone:normalizedPhone,options:{data:{name,email}}});if(error)throw error;setSent(true)}else{const{error}=await supabase.auth.verifyOtp({phone:normalizedPhone,token:token.trim(),type:'sms'});if(error)throw error}}catch(e){const message=String(e.message||'');setError(message.toLowerCase().includes('unsupported phone provider')?'Your number is valid, but SMS login is not configured for this Supabase project. The administrator must enable Phone Auth and configure an SMS provider.':message)}finally{setBusy(false)}}
  return <div className="auth-page"><div className="auth-card"><Logo/><p className="eyebrow">SECURE ACCESS</p><h1>{sent?'Enter your code':'Welcome to Lastbornk'}</h1><p>{sent?`We sent a six-digit code to ${phone}`:'Sign in with your Nigerian phone number.'}</p><form onSubmit={submit}>{!sent&&<><label><span>Your name</span><input required value={name} onChange={e=>setName(e.target.value)} placeholder="Tomi Aigbe"/></label><label><span>Email for payment receipts</span><input required type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="tomi@example.com"/></label></>}<label><span>{sent?'Verification code':'Phone number'}</span><input required type="tel" inputMode="numeric" autoComplete={sent?'one-time-code':'tel'} value={sent?token:phone} onChange={e=>sent?setToken(e.target.value):setPhone(e.target.value)} placeholder={sent?'123456':'+2349127520981'}/></label>{error&&<p className="form-error">{error}</p>}<button className="primary wide" disabled={busy}>{busy?<Loader2 className="spin"/>:sent?'Verify and continue':'Send login code'}</button></form>{sent&&<button className="text-btn" onClick={()=>setSent(false)}>Use another number</button>}</div></div>
}
